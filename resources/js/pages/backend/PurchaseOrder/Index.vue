    ...
    <div class="p-6 bg-bgMain min-h-screen">
      ...
      <div class="mb-4 flex items-center gap-4">
        <Filter
          ...
          buttonClass="bg-primary text-white px-4 py-2 rounded"
        />
        <button
      ...
      <div class="bg-bgCard rounded-xl shadow-card p-6">
        <table class="w-full border-border rounded-lg overflow-hidden">
          ...
          <tbody>
            ...
            >
              <td class="p-3 border-y text-start">{{ (currentPage-1)*perPage + index + 1 }}</td>
...
export default defineComponent({
  components: { BackendLayout, PurchaseOrderCreate, PurchaseOrderEdit, Filter, Pigination },
  setup() {
    ...
    const fetch = async (page = 1) => {
      ...
      const res = await axios.get("/admin/purchase-order/data", {
        params: { search: filterText.value, page, per_page: perPage.value },
      });

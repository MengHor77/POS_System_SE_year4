<template>
  ...
  >
    ...
    <div class="flex items-center justify-between p-6 border-b border-darkSoft">
      ...
      <button @click="$emit('toggle')" class="hover:text-white transition">
      ...
      <div v-for="item in menuItems" :key="item.label" class="flex flex-col">
        ...
        >
          ...
          >
            ...
            <!-- Notification Badge -->
            <span
              v-if="item.label === 'Notification' && (item.count ?? 0) > 0"
          ...
          <span v-else-if="!collapsed && !item.route">{{ item.label }}</span>
        </div>
      </div>
    </nav>
...
export default defineComponent({
  ...
  setup() {
    ...
    const menuItems = reactive([
      ...
      { icon: "fas fa-box-open", label: "Products", route: "/admin/product" },
      { icon: "fas fa-bell", label: "Notification", route: "/admin/notification", count: 0 },
      ...
      { icon: "fas fa-users", label: "Cashiers", route: "/admin/cashier" },
    ...
    const fetchNotificationCount = async () => {
      try {
        ...
        const notification = menuItems.find(i => i.label === "Notification");

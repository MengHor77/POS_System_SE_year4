<template>
  <header class="bg-white shadow px-6 py-4 flex justify-between items-center no-print">
    ...
    <div class="flex items-center gap-6">
      <div class="flex items-center gap-3 px-4 py-2 bg-bgMain rounded-full border border-border">
        <div class="w-8 h-8 bg-primary/10 rounded-full flex items-center justify-center">
            <i class="fas fa-user text-primary text-sm"></i>
        </div>
        
        <div class="flex flex-col">
          <span v-if="loading" class="h-4 w-20 bg-gray-200 animate-pulse rounded"></span>
          <span v-else class="font-bold text-gray-800 leading-none">
            {{ cashier?.name || 'Guest' }}
          </span>
          <span class="text-[10px] uppercase tracking-widest text-muted font-bold">Cashier</span>
        </div>
      </div>
